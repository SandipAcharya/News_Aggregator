    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isOtpMode) {
                ...
                setIsOtpMode(false);
                setIsLoginMode(true);
                // Optionally auto-login, but requiring them to click login is fine
            ...
            // If the backend says unverified, switch to OTP mode
        ...
        <div className="min-h-screen bg-background flex flex-col justify-center items-center px-6 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-surface/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl border border-border relative z-10">
                <div className={`transition-opacity duration-1000 ease-in-out ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
                    <div className="text-center mb-8">
                        ...
                        <h2 className="text-3xl font-bold text-text-main mb-2">
                            {isOtpMode ? 'Verify your email' : (isLoginMode ? 'Welcome back' : 'Create an account')}
                        </h2>
                        <p className="text-sm text-text-muted">
                            {isOtpMode ? 'Enter the 6-digit code sent to your email.' : 'Sign in to customize your news feed.'}
                        </p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    >
                        {isLoading ? 'Processing...' : (isOtpMode ? 'Verify Email' : (isLoginMode ? 'Sign in' : 'Create account'))}
                    </button>
                </form>
